...
export default function Dashboard() {
    ...
    async function createProject(e) {
        e.preventDefault();
        // temporary client-only add (we’ll switch to POST next step)
        ...
        <div className="min-h-screen bg-gray-100 flex">
            ...
            <aside className="w-64 bg-white shadow-md hidden md:block">
                ...
                <nav className="px-2 space-y-1">
                    {["Overview", "Projects", "Tasks", "Calendar", "Settings"].map((i) => (
                        <a key={i} className="block px-3 py-2 rounded-lg hover:bg-gray-100 text-gray-700 cursor-pointer">
                            {i}
                        </a>
                    ))}
                </nav>
            ...
            {showNew && (
                <div className="fixed inset-0 bg-black/30 flex items-center justify-center p-4">
                    <form onSubmit={createProject} className="bg-white rounded-2xl shadow-xl w-full max-w-md p-5 space-y-3">
                        ...
                        <div className="flex justify-end gap-2 pt-1">
                            <button type="button" onClick={() => setShowNew(false)} className="px-3 py-2 rounded-lg border">Cancel</button>
                            <button type="submit" className="px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white">Create</button>
                        </div>
                    </form>
                </div>
            )}
